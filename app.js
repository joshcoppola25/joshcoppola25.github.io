...
function showAllVideos() {
  ...
  for (let i = 0; i < videosList.length; i++) {
    ...
    document.querySelector("div#results").innerHTML += generateRowHTML(videosList[i]);
...
function searchVideos() {
  ...
  document.querySelector("div#results").innerHTML = "";
  let search = document.querySelector("#txt-search").value;
  ...
  if (search === "") {
    ...
    for (let i = 0; i < videosList.length; i++) {
      ...
      if (videosList[i].title.toLowerCase().includes(search) || videosList[i].author.toLowerCase().includes(search) || videosList[i].description.toLowerCase().includes(search)) {
    ...
    if (count === 0) {
      ...
      document.querySelector("div#results").innerHTML = `<p>No videos found with the keyword ${search}`;
...
document.querySelector("#btn-search").addEventListener("click", searchVideos);